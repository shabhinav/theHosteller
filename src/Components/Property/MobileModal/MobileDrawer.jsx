...
function MobileDrawer({ item, setState }) {
  ...
  const [hostelImages, sethostelImage] = React.useState([]);

  React.useEffect(() => {
    if (
      state?.search?.searchedHostelDetails?.getHostelDetails?.images?.length
    ) {
      let images =
        state?.search?.searchedHostelDetails?.getHostelDetails?.images?.map(
          (item) => item?.image
        );
      sethostelImage(images);
    }
  }, []);
